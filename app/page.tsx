...
// Per-user localStorage key — data is namespaced by user ID so accounts never bleed
...
export default function Home() {
  ...
  // ── SECURE DATA LOAD ──
  // Clears all state first, then loads only the current user's data.
  // Never falls back to localStorage from another user.
  useEffect(() => {
    // Always reset to blank state immediately on user change
    setSets([]); setArchivedSets([]); setSetsCreated(0); setStreak(0)
    setThemeName('pink'); setSubjects([])

    async function loadData() {
      if (!user) return // signed out — stay blank
      ...
      try {
        ...
        if (row?.data) {
          const d = row.data
          setSets(d.sets || [])
          setArchivedSets(d.archivedSets || [])
          setSetsCreated(d.setsCreated || 0)
      ...
      // Fall back to THIS user's localStorage key only
  ...
  // ── SECURE SAVE ──
  // Always saves to user-scoped localStorage key + Supabase by user ID.
  async function save(newSets, newArchived, newCreated, newStreak, lastDate) {
    ...
    // Only write to this user's localStorage key
    if (user) {
      const key = localKey(user.id)
      if (key) {
  ...
  function finishStudy(score, updatedCards) {
    const today = new Date().toDateString()
    const newStreak = streak + (streak === 0 || true ? 1 : 0)
    // Only increment once per day — check against the streak date we track
    const key = localKey(user?.id)
    ...
    try {
      ...
      setSubjects(prev => {
        ...
        newSubjects.forEach(ns => {
          const cleaned = {
            ...ns,
            knowledgePercent: typeof ns.knowledgePercent === 'number' ? ns.knowledgePercent : parseInt(ns.knowledgePercent) || 0
          }
          ...
          if (existingIdx >= 0) {
            merged[existingIdx] = { ...merged[existingIdx], ...cleaned }
          } else {
            merged.push(cleaned)
          }
        })
  ...
  useEffect(() => {
    if (screen === 'learn' && sets.length > 0 && subjects.length === 0 && !loadingSubjects) {
      detectSubjects(sets)
    }
  ...
  async function loadVideos(subject) {
    ...
    setVideos([]); setActiveVideo(null); setVideoQuestions([])
    setResources([]); setVideoScore(null)
    setLoadingVideos(true)
    try {
      const [videoRes, resourceRes] = await Promise.all([
        fetch('/api/learn', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'search_videos', subject: subject.subject, weakAreas: subject.weakAreas })
        }),
        fetch('/api/learn', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'search_resources', subject: subject.subject, weakAreas: subject.weakAreas })
        })
      ])
  ...
  async function openVideo(video, subjectOverride) {
    ...
    const inProgressSet = {
      ...
      date: new Date().toDateString(), score: null,
      videoId: video.id,
      videoProgress: { answered: 0, total: 0, inProgress: true },
      color: C.accent
    }
    const newSets = [inProgressSet, ...sets.filter(s => s.videoId !== video.id)].slice(0, 12)
    const newCreated = setsCreated + 1
    setSets(newSets); setSetsCreated(newCreated)
    save(newSets, archivedSets, newCreated, streak, null)

    try {
      let transcriptSegments = []
      try {
        const tRes = await fetch('/api/learn', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'get_transcript', videoId: video.id })
        })
        const tData = await tRes.json()
        transcriptSegments = tData.transcript || []
      } catch (e) { console.log('Transcript unavailable') }
      ...
      if (transcriptSegments.length > 0) {
        for (let i = 0; i < numQuestions; i++) {
          ...
          if (chunkText) {
            ...
            const secs = Math.round(chunkStart % 60)
            transcriptContext += `[${mins}:${secs.toString().padStart(2,'0')}] ${chunkText}\n`
          }
        }
      }
      ...
      const res = await fetch('/api/learn', {
        ...
        body: JSON.stringify({
          ...
          subject: activeSubject?.subject || 'General',
          videoTitle: video.title,
          weakAreas: activeSubject?.weakAreas || [],
      ...
      setVideoTotal(questions.length)

      const updatedSets = newSets.map(s => s.videoId === video.id
        ? {...s, count: questions.length, cards: questions.map(q=>({q:q.q,a:q.answer})), videoProgress: {answered:0, total:questions.length, inProgress:true}}
        : s)
      setSets(updatedSets)
      save(updatedSets, archivedSets, newCreated, streak, null)
  ...
  function answerVideoQuestion(option) {
    ...
    const answeredSoFar = videoCorrect + (correct ? 1 : 0)
    const currentIdx = videoQuestions.findIndex(q => q.q === currentQuestion.q)
    const answeredCount = currentIdx + 1
    const currentScore = Math.round((answeredSoFar / answeredCount) * 100)

    setSets(prev => prev.map(s => s.videoId === activeVideo?.id
      ? {...s, score: currentScore, videoProgress: {answered: answeredCount, total: videoQuestions.length, inProgress: answeredCount < videoQuestions.length}}
      : s))
    ...
    if (currentIdx === videoQuestions.length - 1) {
      ...
      videoQuestions.forEach((q, i) => {
        if (i === currentIdx) {
          if (correct) correctQs.push(q)
          else incorrectQs.push({...q, userAnswer: option})
        }
      })

      const finalSets = sets.map(s => s.videoId === activeVideo?.id
        ? {...s, score: finalScore, videoProgress: {answered: videoQuestions.length, total: videoQuestions.length, inProgress: false}}
        : s)
      setSets(finalSets)
      save(finalSets, archivedSets, setsCreated, streak, null)
      ...
      if (selectedSubject) {
        ...
        fetch('/api/learn', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            action: 'analyze_video_results',
            subject: selectedSubject.subject,
            correct: correctQs,
            incorrect: incorrectQs,
            currentKnowledge: subjectData?.knowledgePercent || 50,
            currentWeak: subjectData?.weakAreas || [],
            currentStrong: subjectData?.strongAreas || []
          })
        }).then(r => r.json()).then(analysis => {
          if (analysis.newKnowledgePercent !== undefined) {
            setSubjects(prev => prev.map(s => s.subject === selectedSubject.subject ? {
              ...s,
              knowledgePercent: analysis.newKnowledgePercent,
              weakAreas: analysis.newWeakAreas || s.weakAreas,
              strongAreas: analysis.newStrongAreas || s.strongAreas,
              analysis: analysis.analysis || s.analysis
            } : s))
          }
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/generate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: inputText, type: outputType, count: cardCount })
      })
      const data = await res.json()
  ...
  return (
    <div style={{display:'flex', height:'100vh', background:C.bg, color:C.text, fontFamily:"'Inter', system-ui, sans-serif", transition:'background .2s, color .2s'}}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&family=DM+Serif+Display@0;1&family=Outfit:wght@300;400;500&display=swap');
        * { box-sizing: border-box; }
        .card-scene{width:100%;height:360px;perspective:1200px;cursor:pointer;margin-bottom:16px;}
        ...
        .card-face{position:absolute;width:100%;height:100%;backface-visibility:hidden;-webkit-backface-visibility:hidden;border-radius:16px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px;text-align:center;box-sizing:border-box;}
        ...
        .set-card{background:${C.s1};border-radius:16px;padding:16px;position:relative;cursor:pointer;transition:all .15s;border:2px solid ${C.border};}
        ...
        .dropdown{position:absolute;top:38px;right:12px;background:${C.s2};border:1px solid ${C.border2};border-radius:12px;overflow:hidden;z-index:100;min-width:130px;box-shadow:0 8px 32px #00000020;}
        .dropdown-item{padding:9px 14px;font-size:13px;cursor:pointer;transition:background .1s;color:${C.text2};}
        ...
        .tab-btn{flex:1;padding:8px 16px;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;border:none;font-family:inherit;transition:all .15s;}
        .inp{width:100%;background:${C.s1};border:2px solid ${C.border2};border-radius:10px;padding:10px 14px;color:${C.text};font-size:13px;outline:none;font-family:inherit;box-sizing:border-box;transition:border-color .15s;}
        .inp:focus{border-color:${C.accent};}
        .inp::placeholder{color:${C.text3};}
        .edit-overlay{position:fixed;inset:0;background:#00000060;display:flex;align-items:center;justify-content:center;z-index:200;backdrop-filter:blur(2px);}
        .edit-modal{background:${C.s1};border:2px solid ${C.border2};border-radius:18px;padding:28px;width:90%;max-width:500px;}
        .sm2-btn{flex:1;padding:12px 6px;border-radius:10px;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;border:none;transition:all .15s;text-align:center;}
        ...
        <div className="edit-overlay" onClick={() => setEditingCard(null)}>
          <div className="edit-modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:'10px'}}>
              {btnPrimary(saveEditCard,'Save changes',{flex:1,padding:'10px'})}
      ...
      {/* Sidebar */}
      <div style={{width:'210px',background:C.s1,borderRight:`1px solid ${C.border}`,padding:'24px 0',display:'flex',flexDirection:'column',flexShrink:0}}>
...
{[['dashboard','Dashboard'],['create','Create'],['study','Study']].map(([id,label]) => (
  <div key={id} className={`nav-item ${screen===id?'active':'inactive'}`}
    onClick={() => {
      setHistory([])
      setScreen(id)
      if(id!=='study') restart()
...
<div className={`nav-item ${screen==='learn'?'active':'inactive'}`}
  onClick={() => {
    setHistory([])
    setScreen('learn')
    restart()
    if(sets.length > 0 && subjects.length === 0) detectSubjects(sets)
  }}
      ...
      <div style={{flex:1,overflowY:'auto',display:'flex',flexDirection:'column'}}>
        ...
        {screen==='dashboard'&&(
          <div style={{padding:'36px 40px',width:'100%'}}>
            <h1 style={{fontSize:'34px',marginBottom:'4px',...serifStyle}}>Dashboard</h1>
            <p style={{fontSize:'13px',color:C.text3,marginBottom:'32px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'36px'}}>
              {[{label:'Sets created',val:setsCreated},{label:'Cards studied',val:flashcardStudied},{label:'Day streak',val:streak}].map(s => (
                <div key={s.label} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'16px 20px'}}>
                  <div style={{...serifStyle,fontSize:'13px',color:C.text3,marginBottom:'8px'}}>{s.label}</div>
                  <div style={{fontSize:'28px',fontWeight:'300',color:C.text,...serifStyle}}>{s.val}</div>
                </div>
              ))}
            </div>

            {!user ? (
              <div style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'48px',textAlign:'center'}}>
                <div style={{fontSize:'15px',fontWeight:'500',marginBottom:'6px'}}>Sign in to get started</div>
                <div style={{fontSize:'13px',color:C.text3,marginBottom:'20px'}}>Your sets are saved securely to your account.</div>
                <SignInButton mode="modal">
                  <button style={{padding:'10px 24px',background:C.accent,border:'none',borderRadius:'8px',color:'white',fontSize:'13px',cursor:'pointer',fontFamily:'inherit',fontWeight:'500'}}>Sign in</button>
                </SignInButton>
              </div>
              ...
              <>
                {sets.length>0&&(
                  <>
                    <div style={{...serifStyle,fontSize:'13px',color:C.text3,marginBottom:'14px'}}>Study sets</div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px'}} ref={menuRef}>
                      {sets.map((s,i)=>{
                        const due = s.type==='video'
                          ? (s.videoProgress?.inProgress ? (s.videoProgress.total - s.videoProgress.answered) : 0)
                          : s.type==='flashcards'
                          ? (s.cards||[]).filter(c => !c.nextReview || c.nextReview<=new Date().toDateString()).length
                          : 0
                        return (
                            ...
                            {renamingIndex===i ? (
                              <div>
                                ...
                                <div style={{display:'flex',gap:'6px',marginTop:'8px'}}>
                                  {btnPrimary(()=>submitRename(i),'Save',{padding:'5px 12px',fontSize:'12px'})}
                                  {btn(()=>setRenamingIndex(null),'Cancel',{padding:'5px 12px',fontSize:'12px'})}
                                </div>
                              </div>
                            ) : (
                              <div onClick={async () => {
                                ...
                                {s.type==='video'&&s.videoId&&(
                                  <div style={{width:'85px',height:'58px',flexShrink:0,borderRadius:'6px',overflow:'hidden',margin:'-2px 0'}}>
                                    <img src={`https://img.youtube.com/vi/${s.videoId}/mqdefault.jpg`} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                                  </div>
                                )}
                                <div style={{flex:1,minWidth:0}}>
                                  <div style={{fontSize:'13px',fontWeight:'500',marginBottom:'4px',color:C.text,lineHeight:'1.3'}}>{s.title}</div>
                                  <div style={{fontSize:'11px',color:C.text3}}>{s.date}</div>
                                  {s.score!==null&&s.score!==undefined&&(
                                    <div style={{height:'2px',background:C.s3,borderRadius:'2px',marginTop:'10px'}}>
                        ...
                        {archivedSets.map((s,i)=>(
                          <div key={i} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'14px',opacity:0.6}}>
                            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'8px'}}>
                              <span style={{...setLabelStyle,color:C.text3}}>{s.typeLabel}</span>
                              <button onClick={()=>unarchiveSet(i)} style={{background:'transparent',border:`1px solid ${C.border2}`,borderRadius:'6px',padding:'2px 8px',color:C.text2,fontSize:'11px',cursor:'pointer',fontFamily:'inherit'}}>Restore</button>
                            </div>
        ...
        {/* CREATE */}
        {screen==='create'&&(
          <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',padding:'36px 40px'}}>
            <div style={{width:'100%',maxWidth:'800px'}}>
              <div style={{textAlign:'center',marginBottom:'28px'}}>
                <h1 style={{fontSize:'32px',marginBottom:'6px',...serifStyle}}>Create</h1>
              ...
              <div style={{display:'flex',gap:'4px',marginBottom:'24px',background:C.s1,padding:'3px',borderRadius:'10px',border:`2px solid ${C.border}`}}>
              ...
              {createTab==='ai'&&(
                <>
                  <textarea value={inputText} onChange={e=>setInputText(e.target.value)} placeholder="Paste your notes, textbook excerpt, or any text here..." style={{width:'100%',minHeight:'220px',marginBottom:'16px',lineHeight:'1.7',fontSize:'15px'}}/>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'20px'}}>
                    {[['flashcards','Flashcards','Terms & definitions'],['quiz','Quiz','Multiple choice'],['sheet','Study sheet','Key points']].map(([id,name,desc])=>(
                      <div key={name} onClick={()=>setOutputType(id)} style={{background:outputType===id?C.accentDim:C.s1,border:`2px solid ${outputType===id?C.accent:C.border}`,borderRadius:'12px',padding:'16px 12px',textAlign:'center',cursor:'pointer',transition:'all .15s'}}>
                        <div style={{fontSize:'14px',fontWeight:'500',color:outputType===id?C.accentText:C.text,marginBottom:'4px'}}>{name}</div>
                        <div style={{fontSize:'12px',color:C.text3}}>{desc}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                      <label style={{fontSize:'14px',color:C.text2}}>{howMany}</label>
                      <input type="number" min="1" max="50" value={cardCount} onChange={e=>setCardCount(e.target.value)} className="inp" style={{width:'65px'}}/>
                    </div>
                    {btnPrimary(generate,loading?'Generating...':'Generate',{padding:'11px 28px',fontSize:'14px',opacity:loading?0.6:1})}
                  </div>
                </>
              )}
              {createTab==='manual'&&(
                <>
                  <input value={manualSetName} onChange={e=>setManualSetName(e.target.value)} placeholder="Set name" className="inp" style={{marginBottom:'16px',fontSize:'14px'}}/>
                  {manualCards.map((c,i)=>(
                    <div key={i} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'12px',padding:'16px',marginBottom:'12px'}}>
                      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}>
                        <div style={{...labelStyle,color:C.text3}}>Card {i+1}</div>
                        {manualCards.length>1&&<button onClick={()=>removeManualCard(i)} style={{background:'transparent',border:'none',color:C.red,cursor:'pointer',fontSize:'12px',fontFamily:'inherit'}}>Remove</button>}
                      </div>
                  ...
                  <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
                    {btn(addManualCard,'+ Add card',{flex:1,textAlign:'center'})}
                    {btnPrimary(saveManualSet,'Save & study',{flex:1,textAlign:'center'})}
                  </div>
                </>
              )}
            </div>
        ...
        {screen==='settings'&&(
          <div style={{padding:'36px 40px',maxWidth:'640px'}}>
            <h1 style={{fontSize:'32px',marginBottom:'4px',...serifStyle}}>Settings</h1>
            <p style={{fontSize:'13px',color:C.text3,marginBottom:'36px'}}>Customize your experience.</p>
            <div style={{marginBottom:'36px'}}>
              <div style={{...labelStyle,color:C.text3,marginBottom:'16px'}}>Account</div>
              <div style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'22px'}}>
                {user ? (
                  <div>
                    <div style={{display:'flex',alignItems:'center',gap:'14px',marginBottom:'20px'}}>
                      {user.imageUrl
                        ? <img src={user.imageUrl} alt="avatar" style={{width:'48px',height:'48px',borderRadius:'50%',objectFit:'cover',border:`2px solid ${C.border2}`}}/>
                        : <div style={{width:'48px',height:'48px',borderRadius:'50%',background:C.accentDim,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px',fontWeight:'600',color:C.accent}}>{(user.firstName?.[0]||'?').toUpperCase()}</div>
                      }
                      <div style={{flex:1}}>
                        {editingName ? (
                          <div style={{display:'flex',gap:'8px',alignItems:'center',marginBottom:'4px'}}>
                            <input value={newDisplayName} onChange={e=>setNewDisplayName(e.target.value)} className="inp" style={{fontSize:'14px',padding:'6px 10px'}} placeholder="Display name" autoFocus
                              onKeyDown={e=>{
                                if(e.key==='Enter'){user.update({firstName:newDisplayName.split(' ')[0],lastName:newDisplayName.split(' ').slice(1).join(' ')});setEditingName(false)}
                                if(e.key==='Escape')setEditingName(false)
                              }}/>
                            <button onClick={()=>{user.update({firstName:newDisplayName.split(' ')[0],lastName:newDisplayName.split(' ').slice(1).join(' ')});setEditingName(false)}} style={{padding:'6px 12px',background:C.accent,border:'none',borderRadius:'6px',color:'white',fontSize:'12px',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap'}}>Save</button>
                            <button onClick={()=>setEditingName(false)} style={{padding:'6px 10px',background:'transparent',border:`1px solid ${C.border2}`,borderRadius:'6px',color:C.text2,fontSize:'12px',cursor:'pointer',fontFamily:'inherit'}}>Cancel</button>
                          </div>
                        ) : (
                          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'2px'}}>
                            <div style={{fontSize:'15px',fontWeight:'600',color:C.text}}>{user.fullName||user.firstName||'User'}</div>
                            <button onClick={()=>{setEditingName(true);setNewDisplayName(user.fullName||user.firstName||'')}} style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:'5px',padding:'2px 8px',color:C.text3,fontSize:'11px',cursor:'pointer',fontFamily:'inherit'}}>Edit</button>
                          </div>
                        )}
                    ...
                    <SignOutButton signOutCallback={()=>{
                      setSets([]); setArchivedSets([]); setSetsCreated(0); setStreak(0)
                      setSubjects([]); setScreen('dashboard')
                    }}>
                      <button style={{padding:'8px 16px',background:'transparent',border:`1px solid ${C.red}50`,borderRadius:'8px',color:C.red,fontSize:'13px',cursor:'pointer',fontFamily:'inherit',fontWeight:'500'}}>Sign out</button>
                    </SignOutButton>
                  </div>
                ) : (
                  <div>
                    ...
                    <SignInButton mode="modal">
                      <button style={{padding:'9px 20px',background:C.accent,border:'none',borderRadius:'8px',color:'white',fontSize:'13px',cursor:'pointer',fontFamily:'inherit',fontWeight:'500'}}>Sign in</button>
                    </SignInButton>
                  </div>
                )}
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'10px'}}>
                {Object.entries(THEMES).map(([key,t])=>(
                  <button key={key} onClick={()=>changeTheme(key)} style={{padding:'0',border:`2px solid ${themeName===key?C.accent:C.border}`,borderRadius:'12px',cursor:'pointer',overflow:'hidden',background:'transparent',fontFamily:'inherit',outline:'none',transition:'border-color .15s'}}>
                    <div style={{background:t.bg,padding:'10px 8px'}}>
                      <div style={{background:t.s1,borderRadius:'4px',padding:'6px',marginBottom:'4px',border:`1px solid ${t.border}`}}>
                        <div style={{height:'4px',background:t.accent,borderRadius:'2px',marginBottom:'3px',width:'60%'}}></div>
                        <div style={{height:'3px',background:t.text3,borderRadius:'2px',width:'80%'}}></div>
                      </div>
                      <div style={{display:'flex',gap:'3px'}}>
                        <div style={{flex:1,height:'3px',background:t.green,borderRadius:'2px'}}></div>
                        <div style={{flex:1,height:'3px',background:t.amber,borderRadius:'2px'}}></div>
                        <div style={{flex:1,height:'3px',background:t.accent,borderRadius:'2px'}}></div>
                      </div>
                    </div>
                    <div style={{padding:'6px 8px',background:t.s1,borderTop:`1px solid ${t.border}`}}>
                      <div style={{fontSize:'11px',fontWeight:'500',color:t.text,textAlign:'center'}}>{t.name}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {!activeVideo ? (
              <>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'4px'}}>
                  <div>
                    <h1 style={{fontSize:'34px',marginBottom:'4px',...serifStyle,textAlign:selectedSubject?'left':'center'}}>{selectedSubject?selectedSubject.subject:'Recall Map'}</h1>
                    <p style={{fontSize:'13px',color:C.text3,marginBottom:'28px'}}>{selectedSubject?'Videos and resources tailored to what you need to review':''}</p>
                  </div>
                  {selectedSubject&&(
                    <button onClick={()=>{setSelectedSubject(null);setVideos([]);setResources([])}} style={{padding:'8px 16px',background:'transparent',border:`1px solid ${C.border2}`,borderRadius:'8px',color:C.text2,fontSize:'13px',cursor:'pointer',fontFamily:'inherit',marginTop:'8px'}}>← All subjects</button>
                  )}
                </div>
                  ...
                  <>
                    {sets.length===0 ? (
                      ...
                      <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'60vh'}}>
                        <div style={{fontSize:'28px',fontWeight:'700',color:'#000',textAlign:'center'}}>Analyzing your sets...</div>
                      </div>
                    ) : (
                      <>
                        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'24px'}}>
                          {subjects.map((s,i)=>(
                            <div key={i} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'20px',cursor:'pointer',transition:'all .15s'}}
                              onClick={()=>loadVideos(s)}>
                                  ...
                                  {openMenu===`subj-${i}`&&(
                                    <div style={{position:'absolute',top:'28px',right:0,background:C.s2,border:`1px solid ${C.border2}`,borderRadius:'10px',overflow:'hidden',zIndex:100,minWidth:'130px',boxShadow:'0 8px 24px #00000020'}}>
                                      ...
                                      <div className="dropdown-item" onClick={()=>{
                                        ...
                                        setArchivedSubjects(prev=>[...prev, sub])
                                        setSubjects(prev=>prev.filter((_,idx)=>idx!==i))
                                        setOpenMenu(null)
                                      }} style={{color:C.amber}}>Archive</div>
                              ...
                              {s.analysis&&<div style={{fontSize:'12px',color:C.text2,marginBottom:'10px',lineHeight:'1.5'}}>{s.analysis}</div>}
                              {s.weakAreas?.length>0&&(
                                <div style={{marginBottom:'4px'}}>
                                  <span style={{fontSize:'10px',color:'#ef4444',fontWeight:'600'}}>Needs work: </span>
                                  <span style={{fontSize:'10px',color:C.text3}}>{s.weakAreas.slice(0,3).join(', ')}</span>
                                </div>
                              )}
                              {s.strongAreas?.length>0&&(
                                <div style={{marginBottom:'12px'}}>
                                  <span style={{fontSize:'10px',color:'#22c55e',fontWeight:'600'}}>Strong: </span>
                                  <span style={{fontSize:'10px',color:C.text3}}>{s.strongAreas.slice(0,2).join(', ')}</span>
                                </div>
                              )}
                        ...
                        <button onClick={()=>detectSubjects(sets)} disabled={loadingSubjects}
                          style={{background:'transparent',border:`1px solid ${C.border2}`,borderRadius:'8px',padding:'8px 16px',color:C.text3,fontSize:'12px',cursor:'pointer',fontFamily:'inherit',opacity:loadingSubjects?0.6:1}}>
                            ...
                            {showArchivedSubjects&&(
                              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px'}}>
                                {archivedSubjects.map((s,i)=>(
                                  <div key={i} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'20px',opacity:0.6}}>
                                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'12px'}}>
                                      <div style={{fontSize:'15px',fontWeight:'600',color:C.text,...serifStyle}}>{s.subject}</div>
                                      <button onClick={()=>{
                                        setSubjects(prev=>[...prev,s])
                                        setArchivedSubjects(prev=>prev.filter((_,idx)=>idx!==i))
                                      }} style={{background:'transparent',border:`1px solid ${C.border2}`,borderRadius:'6px',padding:'2px 8px',color:C.text2,fontSize:'11px',cursor:'pointer',fontFamily:'inherit'}}>Restore</button>
                                    </div>
                                    <div style={{fontSize:'12px',color:C.text3}}>{s.knowledgePercent}% knowledge</div>
                                  </div>
                                ))}
                              </div>
                            )}
                ...
                {selectedSubject&&(
                  <>
                    <div style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'14px',padding:'20px',marginBottom:'20px'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'14px'}}>
                        <div>
                          <div style={{fontSize:'13px',fontWeight:'600',color:C.text,marginBottom:'4px'}}>Recall breakdown</div>
                          <div style={{fontSize:'12px',color:C.text3,lineHeight:'1.5',maxWidth:'500px'}}>{selectedSubject.analysis}</div>
                        </div>
                        <div style={{fontSize:'22px',fontWeight:'700',color:knowledgeColor(subjects.find(s=>s.subject===selectedSubject.subject)?.knowledgePercent||selectedSubject.knowledgePercent),flexShrink:0,marginLeft:'16px'}}>
                          {subjects.find(s=>s.subject===selectedSubject.subject)?.knowledgePercent||selectedSubject.knowledgePercent}%
                        </div>
                      </div>
                      <div style={{height:'8px',background:C.s3,borderRadius:'4px',marginBottom:'14px'}}>
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                        {selectedSubject.weakAreas?.length>0&&(
                          <div style={{background:C.redDim,border:`1px solid ${C.red}20`,borderRadius:'10px',padding:'12px'}}>
                            <div style={{fontSize:'11px',color:'#ef4444',fontWeight:'600',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'.06em'}}>Needs work</div>
                            {selectedSubject.weakAreas.map((w,i)=>(
                              <div key={i} style={{fontSize:'12px',color:C.text2,marginBottom:'3px'}}>· {w}</div>
                            ))}
                          </div>
                        )}
                        {selectedSubject.strongAreas?.length>0&&(
                          <div style={{background:C.greenDim,border:'1px solid #22c55e20',borderRadius:'10px',padding:'12px'}}>
                            <div style={{fontSize:'11px',color:'#22c55e',fontWeight:'600',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'.06em'}}>Strong</div>
                            {selectedSubject.strongAreas.map((w,i)=>(
                              <div key={i} style={{fontSize:'12px',color:C.text2,marginBottom:'3px'}}>· {w}</div>
                            ))}
                          </div>
                        )}
                    ...
                    {resources.length>0&&(
                      <div style={{marginBottom:'24px'}}>
                        <div style={{...serifStyle,fontSize:'14px',color:C.text,marginBottom:'12px'}}>Recommended resources — focused on your weak areas</div>
                        <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'10px'}}>
                          {resources.map((r,i)=>(
                            <a key={i} href={r.url} target="_blank" rel="noopener noreferrer"
                              style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'12px',padding:'14px',textDecoration:'none',display:'block',transition:'all .15s'}}>
                              ...
                              <div style={{fontSize:'11px',color:C.text3,marginBottom:'6px',lineHeight:'1.4'}}>{r.description}</div>
                              {r.focusArea&&<div style={{fontSize:'10px',color:'#ef4444',fontWeight:'500'}}>Addresses: {r.focusArea}</div>}
                            </a>
                          ))}
                        </div>
                      </div>
                    )}
                    <div style={{...serifStyle,fontSize:'14px',color:C.text,marginBottom:'12px'}}>
                      {selectedSubject.weakAreas?.length>0 ? `Videos focused on: ${selectedSubject.weakAreas.slice(0,2).join(', ')}` : 'Recommended videos'}
                    </div>
                    {loadingVideos ? (
                      <div style={{textAlign:'center',padding:'48px',color:C.text3}}><div style={{fontSize:'14px'}}>Finding videos...</div></div>
                    ) : (
                      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px'}}>
                        {videos.map((v,i)=>(
                          <div key={i} onClick={()=>{navigate('learn');openVideo(v)}}
                            style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'14px',overflow:'hidden',cursor:'pointer',transition:'all .15s'}}>
                            <div style={{position:'relative'}}>
                              <img src={v.thumbnail} alt={v.title} style={{width:'100%',height:'140px',objectFit:'cover'}}/>
                              {v.durationLabel&&(
                                <div style={{position:'absolute',bottom:'6px',left:'6px',background:'#000000cc',borderRadius:'4px',padding:'2px 6px',fontSize:'10px',color:'white',fontWeight:'500'}}>
                                  {v.durationLabel}
                                </div>
                              )}
                              <div style={{position:'absolute',inset:0,background:'#00000030',display:'flex',alignItems:'center',justifyContent:'center',opacity:0,transition:'opacity .2s'}}
                                onMouseEnter={e=>e.currentTarget.style.opacity='1'} onMouseLeave={e=>e.currentTarget.style.opacity='0'}>
                                <div style={{width:'44px',height:'44px',background:'#ffffffcc',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px'}}>▶</div>
                              </div>
                            </div>
              ...
              <div style={{maxWidth:'1100px',width:'100%',margin:'0 auto'}}>
                <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'20px'}}>
                  <button onClick={goBack} style={{padding:'7px 14px',background:'transparent',border:`1px solid ${C.border2}`,borderRadius:'8px',color:C.text2,fontSize:'12px',cursor:'pointer',fontFamily:'inherit'}}>← Back</button>
                  <span style={{fontSize:'14px',fontWeight:'500',color:C.text2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{activeVideo.title}</span>
                  {videoScore!==null&&(
                    <span style={{marginLeft:'auto',fontSize:'13px',fontWeight:'600',color:knowledgeColor(videoScore),flexShrink:0}}>Score: {videoScore}%</span>
                  )}
                </div>
                <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
                  <div style={{borderRadius:'14px',overflow:'hidden',border:`2px solid ${C.border}`,background:'#000'}}>
                  ...
                  {currentQuestion&&(
                    <div style={{background:C.s1,border:`2px solid ${C.accent}`,borderRadius:'14px',padding:'24px'}}>
                      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'12px'}}>
                        <div style={{fontSize:'10px',color:C.accent,fontWeight:'600',textTransform:'uppercase',letterSpacing:'.08em'}}>Knowledge check</div>
                        <div style={{fontSize:'11px',color:C.text3}}>Question {videoQuestions.indexOf(currentQuestion)+1} of {videoQuestions.length}</div>
                      </div>
                      <div style={{fontSize:'17px',fontWeight:'500',marginBottom:'20px',lineHeight:'1.6',...serifStyle,color:C.text}}><MathText text={currentQuestion.q}/></div>
                      {questionAnswered ? (
                        <div>
                          <div style={{padding:'14px',borderRadius:'10px',background:questionAnswered==='correct'?'#22c55e18':'#ef444418',border:`1px solid ${questionAnswered==='correct'?'#22c55e40':'#ef444440'}`,marginBottom:'12px'}}>
                            <div style={{fontSize:'14px',fontWeight:'600',color:questionAnswered==='correct'?'#22c55e':'#ef4444',marginBottom:'6px'}}>{questionAnswered==='correct'?'Correct! +3% knowledge':'Incorrect'}</div>
                            {currentQuestion.explanation&&<div style={{fontSize:'13px',color:C.text2,lineHeight:'1.5'}}>{currentQuestion.explanation}</div>}
                          </div>
                          <button onClick={()=>{setCurrentQuestion(null);setQuestionAnswered(null);playVideo()}} style={{width:'100%',padding:'11px',background:C.accent,border:'none',borderRadius:'8px',color:'white',fontSize:'14px',cursor:'pointer',fontFamily:'inherit',fontWeight:'500'}}>Resume video ▶</button>
                        </div>
                      ) : (
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
                          {currentQuestion.options.map((opt,i)=>(
                            <button key={i} onClick={()=>answerVideoQuestion(opt)} style={{padding:'12px 16px',background:C.s2,border:`1px solid ${C.border2}`,borderRadius:'10px',color:C.text,fontSize:'14px',cursor:'pointer',fontFamily:'inherit',textAlign:'left',lineHeight:'1.4'}}>
                              <span style={{color:C.text3,marginRight:'8px',fontWeight:'600'}}>{['A','B','C','D'][i]}.</span><MathText text={opt}/>
                            </button>
                          ))}
                        </div>
                      )}
                      <div style={{marginTop:'14px',height:'3px',background:C.s3,borderRadius:'2px'}}>
        ...
        {screen==='study'&&(
          <div style={{padding:'36px 40px'}}>
            {!activeCards ? (
              <div style={{textAlign:'center',padding:'80px',color:C.text3}}>
              ...
              <div style={{maxWidth:currentType==='quiz'?'860px':'680px',width:'100%',margin:'0 auto'}}>
                <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'28px'}}>
                  {btn(()=>{goBack();restart()},'← Back')}
                  <span style={{fontSize:'14px',fontWeight:'500',color:C.text2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{currentSetName}</span>
                  <span style={{marginLeft:'auto',fontSize:'12px',color:C.text3,flexShrink:0}}>{currentType==='quiz'?`${Object.keys(quizAnswers).length} / ${(activeCards||[]).length}`:`${cardIndex+1} / ${sessionCards.length}`}</span>
                </div>
                <div style={{height:'2px',background:C.s3,borderRadius:'2px',marginBottom:'28px'}}>
                    ...
                    <div className="card-scene" onClick={()=>setFlipped(f=>!f)}>
                      <div className={`card-flipper ${flipped?'flipped':''}`}>
                        <div className="card-face card-front">
                          <div style={{...labelStyle,color:C.text3,marginBottom:'20px'}}>Question</div>
                          <div style={{fontSize:'22px',lineHeight:'1.5',...serifStyle,color:C.text}}><MathText text={card.q}/></div>
                        </div>
                        <div className="card-face card-back">
                          <div style={{...labelStyle,color:C.accent,marginBottom:'20px'}}>Answer</div>
                          <div style={{fontSize:'17px',color:C.text2,lineHeight:'1.7'}}><MathText text={card.a}/></div>
                        </div>
                      </div>
                    </div>
                    <button onClick={()=>startEditCard(cardIndex)} style={{padding:'6px 12px',background:'transparent',border:`1px solid ${C.border}`,borderRadius:'6px',color:C.text3,fontSize:'12px',cursor:'pointer',fontFamily:'inherit',marginBottom:'14px'}}>Edit card</button>
                    {flipped ? (
                      ...
                      <div style={{textAlign:'center',color:C.text3,fontSize:'13px',padding:'14px',background:C.s1,borderRadius:'10px',border:`1px solid ${C.border}`}}>Flip the card to rate your recall</div>
                    )}
                  </>
                )}
                {currentType==='quiz'&&!quizSubmitted&&(
                  <div>
                    {(activeCards||[]).map((c,i)=>(
                      <div key={i} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'14px',padding:'24px',marginBottom:'14px'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}}>
                          <div style={{...labelStyle,color:C.text3}}>Question {i+1}</div>
                          <button onClick={()=>{setEditingCard(i);setEditQ(c.q);setEditA(c.a)}} style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:'5px',padding:'3px 10px',color:C.text3,fontSize:'11px',cursor:'pointer',fontFamily:'inherit'}}>Edit</button>
                        </div>
                        <div style={{fontSize:'16px',fontWeight:'500',marginBottom:'18px',lineHeight:'1.5',...serifStyle}}><MathText text={c.q}/></div>
                        {(c.options||[c.a]).map((opt,j)=>(
                          <button key={j} className={`opt-btn ${quizAnswers[i]===opt?'selected':''}`} onClick={()=>setQuizAnswers(a=>({...a,[i]:opt}))}>
                            <span style={{color:C.text3,marginRight:'12px',fontSize:'13px'}}>{['A','B','C','D'][j]}.</span><MathText text={opt}/>
                          </button>
                        ))}
                      </div>
                    ))}
                    {btnPrimary(submitQuiz,'Submit',{width:'100%',padding:'13px',fontSize:'15px',opacity:Object.keys(quizAnswers).length<(activeCards||[]).length?0.4:1})}
                  </div>
                )}
                {currentType==='sheet'&&!done&&(
                  <div>
                    {(activeCards||[]).map((c,i)=>(
                      <div key={i} style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'14px',padding:'22px',marginBottom:'10px',borderLeft:`3px solid ${C.green}`}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'10px'}}>
                          <div style={{fontSize:'15px',fontWeight:'500',color:C.text,...serifStyle}}><MathText text={c.q}/></div>
                          <button onClick={()=>{setEditingCard(i);setEditQ(c.q);setEditA(c.a)}} style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:'5px',padding:'3px 10px',color:C.text3,fontSize:'11px',cursor:'pointer',fontFamily:'inherit',flexShrink:0,marginLeft:'12px'}}>Edit</button>
                        </div>
                        <div style={{fontSize:'14px',color:C.text2,lineHeight:'1.8'}}><MathText text={c.a}/></div>
                      </div>
                    ))}
                    {btnPrimary(()=>finishStudy(100,activeCards),'Mark as studied',{width:'100%',padding:'12px',marginTop:'8px'})}
                  </div>
                )}
                {currentType==='quiz'&&quizSubmitted&&(
                  <div style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'40px',textAlign:'center'}}>
                    <div style={{fontSize:'28px',marginBottom:'6px',...serifStyle}}>Quiz complete</div>
                    <div style={{fontSize:'15px',color:C.text2,marginBottom:'28px'}}><span style={{color:'#22c55e',fontWeight:'600'}}>{quizScore}</span> of <span style={{fontWeight:'600'}}>{(activeCards||[]).length}</span> correct</div>
                    <div style={{marginBottom:'28px',textAlign:'left'}}>
                      {(activeCards||[]).map((c,i)=>(
                        <div key={i} style={{background:quizAnswers[i]===c.a?'#22c55e12':'#ef444412',border:`1px solid ${quizAnswers[i]===c.a?'#22c55e30':'#ef444430'}`,borderRadius:'10px',padding:'14px 16px',marginBottom:'8px'}}>
                          <div style={{fontSize:'14px',fontWeight:'500',marginBottom:'4px'}}><MathText text={c.q}/></div>
                          <div style={{fontSize:'13px',color:quizAnswers[i]===c.a?'#22c55e':'#ef4444'}}>{quizAnswers[i]===c.a?'Correct':`You answered: ${quizAnswers[i]}`}</div>
                          {quizAnswers[i]!==c.a&&<div style={{fontSize:'13px',color:'#22c55e',marginTop:'3px'}}>Correct: <MathText text={c.a}/></div>}
                        </div>
                      ))}
                    </div>
                ...
                {currentType==='flashcards'&&done&&(
                  <div style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'40px',textAlign:'center'}}>
                    <div style={{fontSize:'28px',marginBottom:'6px',...serifStyle}}>Session complete</div>
                    <div style={{fontSize:'13px',color:C.text3,marginBottom:'28px'}}>{got} recalled · {learning} to review</div>
                    <div style={{display:'flex',gap:'10px',justifyContent:'center',marginBottom:'24px'}}>
                      <div style={{background:'#22c55e12',border:'1px solid #22c55e30',borderRadius:'12px',padding:'16px 32px'}}>
                        <div style={{fontSize:'28px',...serifStyle,color:'#22c55e'}}>{got}</div>
                        <div style={{...labelStyle,color:C.text3,marginTop:'4px'}}>Recalled</div>
                      </div>
                      <div style={{background:'#ef444412',border:'1px solid #ef444430',borderRadius:'12px',padding:'16px 32px'}}>
                        <div style={{fontSize:'28px',...serifStyle,color:'#ef4444'}}>{learning}</div>
                    ...
                    {weakCardsCount>0&&(
                      <div style={{background:'#f59e0b12',border:'1px solid #f59e0b30',borderRadius:'10px',padding:'14px 16px',marginBottom:'16px',textAlign:'left'}}>
                        ...
                        <button onClick={studyWeakCards} disabled={generatingReview} style={{width:'100%',padding:'9px',background:'#f59e0b',color:'#000',border:'none',borderRadius:'8px',fontSize:'12px',fontWeight:'600',cursor:'pointer',opacity:generatingReview?0.6:1,fontFamily:'inherit'}}>
                    ...
                    {btn(()=>{setScreen('dashboard');restart()},'Back to dashboard',{padding:'10px 24px'})}
                  </div>
                )}
                {currentType==='sheet'&&done&&(
                  <div style={{background:C.s1,border:`2px solid ${C.border}`,borderRadius:'16px',padding:'40px',textAlign:'center'}}>
                    <div style={{fontSize:'28px',marginBottom:'6px',...serifStyle}}>Sheet reviewed</div>
                    <div style={{fontSize:'13px',color:C.text3,marginBottom:'24px'}}>All {(activeCards||[]).length} concepts reviewed</div>
                    <div style={{display:'flex',gap:'8px',justifyContent:'center'}}>
